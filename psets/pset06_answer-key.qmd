---
title: "BIOL 220 Problem Set 06: Binomial Test"
subtitle: "Answer Key"
format:
  pdf:
    toc: false
...
A binomial test requires actual count data, not just percentages
...
pbinom(9, size = 20, prob = 0.2, lower.tail = FALSE)

```

Note that you need to do `pbinom(9, ....)` rather than `pbinom(10, ....)` because `pbinom` calculates probability in a "greater than" way rather than a "greater than or equal to" way
::::
...
# note: same p-value if you do `x = 14` because the binomial with p = 0.5
# is symmetric and H_A is two-tailed
binom.test(x = 14, n = 20, p = 0.5)
...
Or you could directly use `pbinom`


```{r}
#| echo: true

lwr_tail <- pbinom(6, size = 20, prob = 0.5) 
upr_tail <- pbinom(13, size = 20, prob = 0.5, lower.tail = FALSE)

# p-val
lwr_tail + upr_tail

# same same if we just do 2 * lwr_tail because symmetry and two-sided test
2 * lwr_tail
