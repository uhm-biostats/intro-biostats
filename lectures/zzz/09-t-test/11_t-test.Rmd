...
```{r echo = FALSE}
sus <- rnorm(15, 720, 223.6)
sus <- (sus - mean(sus)) / sd(sus) 
sus <- sus * 223.6 + 720

res <- rnorm(16, 582, 277.3)
res <- (res - mean(res)) / sd(res) 
res <- res * 277.3 + 582

dat <- data.frame(genotype = rep(c("Susceptible", "Resistant"), c(15, 16)), 
                  n_seeds = c(sus, res))
```

\small
```{r echo = TRUE}
t.test(dat$n_seeds ~ dat$genotype)
```
