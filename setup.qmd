...
for(i in 1:8) {
  assign(sprintf("koa%s", i), 
         image_trim(image_read(sprintf("img/koa/‎koa.‎00%s.png", i))))
...
RStudio is arranged into several panels, each with its own purpose.  You will learn all about these in ["Lab 01: Getting started"](labs/lab01.qmd).  For now, we need to know that the bottom right panel is how we access files and folders on the server.  Click on the folder named for our class "biol220_class."
